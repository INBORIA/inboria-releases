...
## ⚠️ ARCHITECTURE — LIS CECI EN PREMIER AVANT TOUT

L'application NCV Mail est composée de **3 systèmes distincts et indépendants**. Ne les confonds pas :

```
┌─────────────────────────────────────────────────────────────┐
│  1. HOSTINGER  →  ncvmail.com                               │
│     Rôle : Site vitrine marketing UNIQUEMENT                │
│     Contenu : Page d'accueil, tarifs, fonctionnalités       │
│     Tu n'y touches PAS — c'est déjà en ligne et terminé    │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│  2. REPLIT  →  app.ncvmail.com                              │
│     Rôle : L'APPLICATION (dashboard, auth, interface)       │
│     Contenu : React + TypeScript — tout le frontend         │
│     C'est ICI que tu travailles — c'est ta mission          │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│  3. SUPABASE  →  ecdwevvisbrcsomdiqop.supabase.co           │
│     Rôle : Base de données + Authentification               │
│     Contenu : Tables emails, categories, profiles, tasks    │
│     Replit se CONNECTE à Supabase via l'API client          │
│     Tu ne crées PAS de base de données dans Replit          │
└─────────────────────────────────────────────────────────────┘
```

**Résumé en une phrase :**
> Replit affiche l'interface → Supabase stocke les données → Hostinger est la vitrine. Ces 3 outils ne se remplacent pas, ils se complètent.

**Ce que Replit NE DOIT PAS faire :**
- ❌ Créer une base de données locale dans Replit
- ❌ Héberger le site vitrine (c'est Hostinger)
- ❌ Gérer l'authentification en interne (c'est Supabase Auth)

**Ce que Replit DOIT faire :**
- ✅ Construire l'interface React (pages, composants, navigation)
- ✅ Se connecter à Supabase avec `@supabase/supabase-js`
- ✅ Appeler l'API Supabase pour lire/écrire les données
- ✅ Utiliser Supabase Auth pour la connexion/inscription
