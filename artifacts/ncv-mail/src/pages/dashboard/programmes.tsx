...
import { CalendarClock, Trash2, Mail } from "lucide-react";
import { Button } from "@/components/ui/button";
import { useListScheduledEmails, useCancelScheduledEmail, getListScheduledEmailsQueryKey } from "@workspace/api-client-react";
    ...
    <DashboardLayout>
      <div className="p-4 md:p-6 max-w-3xl mx-auto">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h1 className="text-[16px] font-semibold text-white">{t("wave1.scheduledPageTitle")}</h1>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 border border-border rounded-md bg-card">
            <Mail className="w-8 h-8 mx-auto text-[#8b9cb3] mb-2 opacity-50" />
          ...
          <div className="space-y-2">
            {emails.map((e: any) => (
              <div key={e.id} className="border border-border rounded-md bg-card p-3 flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <div className="text-[13px] text-white font-medium truncate">{e.subject || "(sans sujet)"}</div>
                  <div className="text-[11px] text-[#8b9cb3] mt-0.5 truncate">→ {e.recipient}</div>
                  <div className="text-[11px] text-amber-300 mt-1 flex items-center gap-1">
                    <CalendarClock className="w-3 h-3" />
                    {t("wave1.scheduledSentAt", { date: fmt.format(new Date(e.scheduledSendAt)) })}
                  </div>
                </div>
                <Button
                  size="sm"
