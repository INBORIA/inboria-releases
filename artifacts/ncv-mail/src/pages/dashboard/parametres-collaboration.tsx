...
export default function ParametresCollaboration() {
  ...
  const renderProviderCard = (provider: "slack" | "notion", label: string, Icon: any, descKey: string) => {
    const row = findIntegration(provider);
    const available = availability.data?.[provider] ?? false;
      ...
      <div className="p-5 max-w-4xl mx-auto w-full space-y-4">
        ...
        <div>
          <h1 className="text-[16px] font-semibold text-white tracking-tight flex items-center gap-2">
            ...
            {t("settings.hub.collaboration", "Collaboration")}
          </h1>
          <p className="text-[12px] text-[#8b9cb3] mt-0.5">
            {t("settings.hub.collaborationDesc", "Slack, Notion")}
          </p>
        </div>

        <div className="grid gap-3 md:grid-cols-2">
          {renderProviderCard("slack", "Slack", Slack, "integrations.slack.desc")}
          {renderProviderCard("notion", "Notion", FileText, "integrations.notion.desc")}
        </div>
      </div>
    </DashboardLayout>
  );
