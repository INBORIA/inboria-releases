...
import { notifyReadingPaneOpen } from "@/lib/use-reading-pane";
...
export function MailReadingPane({
  ...
  return (
    <aside
      aria-hidden={!open}
      className={`fixed top-0 right-0 bottom-0 z-30 bg-background border-l border-border shadow-2xl flex flex-col transition-transform duration-200 ease-out ${widthClassName} ${open ? "translate-x-0" : "translate-x-full pointer-events-none"}`}
