...
var supabaseAdmin = createClient(url, key);
async function main() {
  const { data: connections, error: connErr } = await supabaseAdmin.from("email_connections").select("id, user_id, provider, email_address");
  if (connErr) {
  ...
  for (const [userId, conns] of byUser.entries()) {
    const { data: emails, error: emailsErr } = await supabaseAdmin.from("emails").select("id, external_id, recipient, shared_mailbox_id").eq("user_id", userId).or("recipient.is.null,recipient.eq.");
    if (emailsErr) {
    ...
    for (const e of emails) {
      ...
      const { error: updErr } = await supabaseAdmin.from("emails").update({ recipient: target.email_address }).eq("id", e.id);
      if (updErr) {
