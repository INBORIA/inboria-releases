...
const MAIL_TEMPLATES: MailTemplate[] = [
  ...
  { direction: "in", daysAgo: 12, subjectPrefix: "Question rapide – ", priority: "moyen", body: (p) => `Bonjour Richard,\n\nPetite question sur le ${p.topic} : pouvez-vous nous confirmer que la livraison est toujours prevue fin juin ?\nNous avons un comite de pilotage interne le 5 juillet et besoin d'un statut clair. URGENT - reponse attendue avant vendredi 23 mai 2026.\n\nMerci,\n${p.contactName}` },
  // Templates ajoutés pour T25/T28/T33/T34 — deadlines + statuts EXPLICITES
  // dans le body (avant Inboria devait deviner, maintenant il extrait littéralement).
  { direction: "in", daysAgo: 4, subjectPrefix: "Statut projet – ", priority: "urgent", body: (p) => `Bonjour Richard,\n\nMerci de nous transmettre un statut écrit du projet "${p.name}" avant LUNDI 19 MAI 2026 17H.\n\nElements attendus :\n- avancement (% realise)\n- prochaines etapes (sem 21)\n- risques identifies\n- budget consomme vs prevu\n\nDeadline ferme : lundi 19/05 17h. Le COPIL hebdomadaire passe en revue ${p.topic} mardi matin.\n\nMerci,\n${p.contactName}` },
  { direction: "in", daysAgo: 2, subjectPrefix: "URGENT - relance – ", priority: "urgent", body: (p) => `Bonjour Richard,\n\nJe me permets de revenir vers vous : nous n'avons pas eu de retour sur "${p.name}" depuis 10 jours.\n\nDEADLINE : reponse attendue AVANT VENDREDI 23 MAI 2026 fin de journée (sinon report du jalon de 3 semaines).\n\nStatut actuel cote ${p.contactName.split(" ")[0]} : EN ATTENTE DE VOUS.\nRisque identifie : decalage planning + impact budget +${eur(2500)}.\n\nMerci de nous donner un signe de vie rapidement.\n\n${p.contactName}\n${p.contactEmail}` },
];
