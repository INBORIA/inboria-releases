...
function wireAutoUpdaterOnce() {
  ...
  autoUpdater.on("error", (err) => {
    ...
    // La vérification automatique échoue en silence (juste loggée) :
    // une coupure réseau passagère ne doit pas interrompre l'utilisateur.
    if (manualUpdateCheck) {
      box({
        type: "error",
        buttons: ["OK"],
        ...
        detail: isNetworkErrorMessage(msg)
          ? "Impossible de joindre le serveur de mises à jour. Vérifiez votre connexion internet, puis réessayez."
          : msg,
      });
    }
    manualUpdateCheck = false;
...
// Erreurs réseau transitoires (DNS/connexion) → message clair, sans jargon.
function isNetworkErrorMessage(msg) {
  return /ERR_NAME_NOT_RESOLVED|ERR_INTERNET_DISCONNECTED|ERR_CONNECTION|ENOTFOUND|ETIMEDOUT|ECONNRESET|ECONNREFUSED|EAI_AGAIN|net::/i.test(
    msg || ""
  );
}
...
if (!gotLock) {
  ...
  app.whenReady().then(() => {
      ...
      Menu.buildFromTemplate([
        {
          ...
          submenu: [
            {
              label: "Connecter mon Vault…",
              click: () => vault.openPairingWindow(),
            },
            {
              label: "Choisir le dossier du Vault…",
              click: () => vault.changeRootFolder(),
            },
            { type: "separator" },
            {
    ...
    // Inboria Vault « Mon ordinateur » : reprend le dépôt si l'appareil est
    // déjà jumelé (réglage local), sinon attend « Connecter mon Vault… ».
    vault.initVault(() => mainWindow);
