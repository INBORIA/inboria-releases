...
- **Follow-ups**: Manages follow-up tracking with CRUD endpoints and status management. **Detection scope (fix 2026-04-28)**: Le détecteur de relances IA (`services/follow-up-detector.ts`) ne propose des suggestions que pour les mails *envoyés depuis Inboria* — filtres `status = "sent"` ET `external_id IS NULL`. Les mails sortants synchronisés depuis Gmail/Outlook/IMAP (où `external_id` vaut `gmail:...`, `imap:...`, etc.) sont exclus pour éviter un volume massif de faux positifs (cas constaté: 9 envois Inboria → 83 suggestions IA). Cleanup migration: `2026_04_28_cleanup_followup_suggestions_for_synced_emails.sql` (UPDATE dismissed_at sur les anciennes suggestions IA pointant vers des mails externes synchronisés). **Self-healing (fix 2026-04-28 #2)**: `cleanupStaleAiFollowups(userId)` tourne à chaque cycle de détection (toutes les 5 min) et marque automatiquement `dismissed_at = NOW()` sur toute suggestion IA active dont le mail lié n'est plus un candidat valide (status changé, external_id réécrit par sync IMAP ultérieure, mail supprimé, suggestion héritée d'une ancienne version). Re-check `external_id` aussi en defense-in-depth juste avant l'INSERT.
...
- **Inboria Memory — Phase 1 (2026-04-30)**: Mémoire contextuelle par contact pour Contact 360°. Tables Supabase (SQL Editor manuel, pas Drizzle): `inboria_facts(kind ∈ {preference,topic,role}, statement, confidence, embedding vector(1536))`, `inboria_episodes(kind ∈ {decision,commitment}, summary, event_date)`, `inboria_signals` (Phase 3). Toggle confidentialité par mailbox: colonnes `inboria_enabled` sur `email_connections` + `shared_mailboxes` (default true). Colonne `inboria_processed_at` sur `emails` pour ne pas retraiter. **Worker** `services/inboria-extractor.ts` (warm-up 60s, batch 20 toutes les 15min, ordre `created_at DESC` pour priorité aux récents): gpt-4o-mini + text-embedding-3-small (1536 dims), prompt FR strict avec kinds figés, parse Zod permissif (filtre items invalides au lieu de tout rejeter). Skip emails: spam, sortants, body<50 chars, mailbox `inboria_enabled=false`, user orphelin (FK auth.users supprimé). **Retry transient** (architect 2026-04-30): erreurs OpenAI réseau/5xx/429 ne marquent PAS `inboria_processed_at` → retry au cycle suivant; erreurs déterministes (JSON malformé, parse Zod, status 4xx) → marquent processed pour éviter coût récurrent. **Endpoint** `GET /api/inboria/context?contactEmail=...&limit=10` (route `routes/inboria-context.ts`, requireAuth via `req.userId!`, scope multi-tenant via `getMemberMailboxIds`: emails persos + shared mailboxes du membre). Schémas OpenAPI ajoutés (tag `inboria`) et hooks/Zod générés via `pnpm --filter @workspace/api-spec run codegen`. Phase 1 reste à compléter: UI Contact 360° (livraison 3) et UI toggle confidentialité (livraison 4).
