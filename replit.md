...
- **Plateforme & écosystème (Vague 4 — Plus plan)**: Bidirectional CRM integrations (HubSpot, Pipedrive) with OAuth, contact/deal sync, and email logging as activities/notes. Public REST API at `/api/v1/*` authenticated via API keys (`Authorization: Bearer ibk_...`) with read/write scopes; outbound webhooks (`webhook_subscriptions`) signed with HMAC-SHA256 (`X-Inboria-Signature: sha256=...`) emit `email.received`, `task.created`, `appointment.created`, `rule.triggered`, `message.received` events to Zapier/Make/n8n. Multi-channel messaging via WhatsApp Business (Meta Cloud API) and SMS (Twilio + Brevo) with unified inbound webhook ingestion into the new `messages` table. New "Plus" pricing tier (€29/month, 15k credits, includes WhatsApp/SMS/CRM/API). UI: `/dashboard/parametres/integrations` page with CRM/Communication/Automation sections, accessible from `/dashboard/parametres`. Migration: `2026_04_24_v4_platform_ecosystem.sql`. SSRF protection on outbound webhooks (blocks loopback/RFC1918/link-local in production). i18n FR/EN/NL/DE/ES.
