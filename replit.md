...
- **Brouillons partagés / co-rédaction temps réel (T005)**: appliquer manuellement `artifacts/api-server/migrations/2026_06_03_shared_drafts.sql` dans Supabase (crée `shared_drafts` + 2 index + RLS scoped organisation via `profiles.organisation_id`). Sans ça, les routes `/api/drafts` répondent en **mode dégradé** (GET → liste vide, POST → 503 « migration non appliquée », PATCH/DELETE no-op) grâce au cache `hasDraftsTable()` (60s) côté backend → le bouton « Brouillon partagé » du composer ne fera rien de visible. Mécanisme : bouton « Brouillon partagé » (icône Users) visible UNIQUEMENT en contexte partagé (`isSharedContext = sharedMailboxId || assignedTo`), dans l'entête du composer de réponse (`EmailDetail.tsx`). À l'activation → POST/GET `/api/drafts` (rejoint un brouillon existant du mail = adopte son contenu, sinon en crée un avec le contenu courant). Synchro live via canal Supabase broadcast `draft-<id>` (debounce broadcast 250ms + persistance PATCH debounce 900ms) + presence des éditeurs (pastilles couleur déterministe). v1 **dernier qui écrit gagne** (pas d'OT/CRDT). Anti-écho : `lastSyncedRef` (JSON {to,cc,subject,body}) + `applyingDraftRef`. Le brouillon est supprimé à l'envoi (`remove()`), conservé à l'annulation (`deactivate()`). Hook réutilisable `src/hooks/use-shared-draft.ts` + helper `src/lib/api-fetch.ts` (`authFetch`/`authJson`, Bearer + `BASE_URL`). i18n FR en fallback inline (batch 43 langues à faire).
