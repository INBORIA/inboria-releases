...
**Why :** `Office.context.ui.openBrowserWindow` (Outlook) **ne préserve PAS le fragment** `#...` — le composeur s'ouvrait vide. Indice de diagnostic : les deep-links qui passent par la query (`?emailId=`, `?from=`) marchent, ceux qui passent par le hash échouent. La query survit aussi à la redirection d'auth (`/login`) car capturée au boot dans `main.tsx` et stockée en sessionStorage avant le routing React.
