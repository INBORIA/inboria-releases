...
**Décalage déploiement (piège vérifié) :** le guide in-app (`openGmailGuide`) et le zip extension servent le fichier `public/` **tel que DÉPLOYÉ** sur le domaine consulté (prod = `https://inboria.com`), pas la source dev. Après édition de la source d'un pont, tant que l'app n'est pas republiée, le bouton « Copier le code »/« Télécharger » du guide donne l'**ancienne** version (placeholder `__INBORIA_BASE__` + ancien transport fragment). Pour dépanner sans redéployer : livrer des fichiers corrigés à la main (substituer `__INBORIA_BASE__` → `https://inboria.com`, et l'URL dev codée en dur de `panel.js` → `https://inboria.com`) — l'endpoint `/api/inboria/draft-handoff` est déjà en prod, donc le pont marche dès qu'il pointe vers prod. Sinon : republier pour que le guide serve la bonne version.
