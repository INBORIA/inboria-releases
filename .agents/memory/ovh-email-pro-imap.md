...
**Why:** the in-app "Autre fournisseur (IMAP)" button in settings prefills the generic `ssl0.ovh.net`, which FAILS for Email Pro boxes. Email Pro boxes must use `pro3.mail.ovh.net:993` (visible in api-server imap-sync logs for working OVH Email Pro connections).
